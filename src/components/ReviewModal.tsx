...
export default function ReviewModal({
  ...
  const handleSubmit = async () => {
    try {
        await createReview({booking_id: bookingId, user_id: userId, rating, comment},token)
        onSuccess()
        onClose() 
    } catch (error:any) {
        console.error("Failed to create review", error)
        setErrorMessage(error?.response?.data?.message || "review failed")
    }
  }
  ...
  return(
    <div className="bg-white rounded-xl shadow-md p-6 w-full max-w-lg mx-auto animate-fadeIn">
        <div className="bg-white p-6 rounded-xl shadow-lg w-full max-w-md space-y-4">
            <h2 className="text-xl font-bold text-[#A88763]">
            Write Your Review
            </h2>

            <div className="flex gap-2">
                {Array.from({length: 5}).map((_, i) => (
                    <FaStar key={i} onClick={() => setRating(i + 1)}
